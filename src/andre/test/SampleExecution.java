...
// Otherwise, openl will generate its own class definition in a separate classloader, resulting in argument type mismatch exception		
		...
		for (int i = 0; i < persons.length; i++) {
			...
			System.out.println(o.get("name") + ", " + o.get("gender") + ", " + o.getWrappedObject().getClass().getName());
		}
		...
		try {
			System.out.println(ValidationPolicy.execute(p.getWrappedObject()));
		} catch (Throwable e) {
		...
		ValueObject pmt = new ValueObject();
		pmt.setClassName("org.openl.generated.beans.Payments")
			.addField("number", "Integer")
			.addField("amounts", "Double[]")
			.addField("interests", "Double[]")
			.build();
		...
		Object array = Array.newInstance(loss1.getWrappedObject().getClass(), 2);
		Object[] losses = (Object[]) array;
		losses[0] = loss1.getWrappedObject();
		losses[1] = loss2.getWrappedObject();
